...
import { BACKEND_API_URL, DOWNLOAD_API_URL } from '../config/api';
...
const Sidebar = ({ onLayerToggle, isThematicMode, onThematicModeToggle, isSidebarOpen }) => {
  ...
  useEffect(() => {
    fetch(`${BACKEND_API_URL}/api/layer_hierarchy`)
      .then((res) => res.json())
      .then((data) => {
        const themeGroups = [];
        const initialState = {};
  ...
  const handleViewAttributes = (layer) => {
    ...
    fetch(`${BACKEND_API_URL}/api/geojson/${layer}`)
      .then((res) => res.json())
      .then((geojson) => {
  ...
  const handleDownloadShp = (layer) => {
    ...
    link.href = `${DOWNLOAD_API_URL}/api/shapefiles/${layer}.zip`;
  ...
  const confirmBatchDownload = () => {
    ...
    const link = document.createElement("a");
    link.href = `${DOWNLOAD_API_URL}/api/shapefiles/batch?layers=${encodeURIComponent(query)}`;
                      ...
                      <ul className="subtheme-dataset-list">
                        ...
                        <li key={dataset.layer_name} className="dataset-item-row">
                          ...
                          <div className="layer-actions-below">
                            ...
                            <button onClick={() => window.open(`${BACKEND_API_URL}/api/metadata_html/${dataset.layer_name}`, "_blank")}>
                              View Metadata
                            </button>
                          </div>
                        </li>
